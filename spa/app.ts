...
// Optional LAYOUT, HEADER, TITLE, NO_FOOTER, ZOOM, MAXWIDTH, AUTOLOOP from layout.js (loaded before this script)
...
declare const AUTOLOOP: { control: string; seconds: number; range?: [number, number] } | undefined;

// Find the first and last indices of values within [lo, hi] (inclusive)
function findRangeIndices(values: (number | string | boolean)[], lo: number, hi: number): [number, number] {
  const start = values.findIndex(v => Number(v) >= lo);
  let end = start;
  for (let i = values.length - 1; i >= start; i--) {
    if (Number(values[i]) <= hi) { end = i; break; }
  }
  return [start, end];
}
...
function initViewer(data: JuliaExportData) {
  ...
  const widgetRefs = new Map<string, { input: HTMLInputElement; valueSpan: HTMLSpanElement }>();
  ...
  if (typeof AUTOLOOP !== 'undefined') {
    ...
    if (widget && refs) {
      ...
      const group = refs.input.closest('.widget-group') as HTMLElement;
      if (group) {
      ...
      controlsContainer.addEventListener('mouseenter', () => {
        paused = true;
        if (group) group.classList.remove('autolooping');
      });
      controlsContainer.addEventListener('mouseleave', () => {
        paused = false;
        if (group) group.classList.add('autolooping');
      });
    }
  }
}
